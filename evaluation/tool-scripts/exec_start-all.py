...
result = subprocess.run(['bash', './tool-scripts/action-home.sh'], stdout=subprocess.PIPE)
...
for i in range(1):    
    ...
    print(result)
    status, launch_state, wait_time = process_result(result)
    if status == 'ok' and launch_state == 'HOT':
        hot_launch_time_twitter.append(wait_time)
    elif status == 'ok' and launch_state == 'COLD':
        cold_launch_time_twitter.append(wait_time)

    use_utils.use_phone_always_swipe_up(USE_DURATION)
    if IF_COUNT_CACHE:
