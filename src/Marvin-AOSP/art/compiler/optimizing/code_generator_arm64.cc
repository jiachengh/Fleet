...
void InstructionCodeGeneratorARM64::HandleFieldGet(HInstruction* instruction,
  ...
  // marvin start
  // if (InputRegisterAt(instruction, 0).GetCode() != OutputCPURegister(instruction).GetCode()) {
...
void InstructionCodeGeneratorARM64::VisitArrayGet(HArrayGet* instruction) {
  ...
  // marvin start
  // if (!(out.IsRegister() && out.reg() == helpers::ARTRegCodeFromVIXL(obj.GetCode()))) {
  //   codegen_->GenerateRestoreStub(obj);
  // }
  // marvin end
}
...
void InstructionCodeGeneratorARM64::VisitArrayLength(HArrayLength* instruction) {
  ...
  // marvin start
  // if (InputRegisterAt(instruction, 0).GetCode() != OutputRegister(instruction).GetCode()) {
  //   codegen_->GenerateRestoreStub(InputRegisterAt(instruction, 0));
  // }
  // marvin end
}
...
void InstructionCodeGeneratorARM64::VisitArraySet(HArraySet* instruction) {
  ...
  // marvin start
  // Note: if we were allowing object arrays to be swapped out, we would want
  // to make sure that the code path for writing to an object array calls
  // GenerateRestoreStub() before MarkGCCard(); however, since we're not
  // allowing object arrays to be swapped out, there should never be a
